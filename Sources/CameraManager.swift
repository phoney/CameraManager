...
extension CaptureContent {
    ...
    public var asData: Data? {
        ...
        case let .imageData(data):
            return data
        case let .asset(asset):
            return getImageData(fromAsset: asset)
        }
    }
...
open class CameraManager: NSObject, AVCaptureFileOutputRecordingDelegate, UIGestureRecognizerDelegate {
    ...
    open var activeVideoStabilisationMode: AVCaptureVideoStabilizationMode {
        if let movieOutput = movieOutput {
            for connection in movieOutput.connections {
                for port in connection.inputPorts where port.mediaType == AVMediaType.video {
    ...
    open var userDidZoomCallback: ((_ zoomScale: CGFloat) -> Void)?

    /// The maximum value of the maxZoomScale
    open var zoomLimit: CGFloat = 6
    ...
    private lazy var frontCameraDevice: AVCaptureDevice? = {
        AVCaptureDevice.videoDevices.first(where: { $0.position == .front })
    }()

    private lazy var backCameraDevice: AVCaptureDevice? = {
        AVCaptureDevice.videoDevices.first(where: { $0.position == .back })
    }()
    ...
    /**
     Zoom in to the requested scale. This should be a value between beginZoomScale and maxZoomScale.
     */
    open func zoom(to scale: CGFloat) {
        _zoom(scale / beginZoomScale)
    }
    ...
    open func askUserForCameraPermission(_ completion: @escaping (Bool) -> Void) {
        AVCaptureDevice.requestAccess(for: AVMediaType.video, completionHandler: { allowedAccess in
            if self.cameraOutputMode == .videoWithMic {
                AVCaptureDevice.requestAccess(for: AVMediaType.audio, completionHandler: { allowedAccess in
    ...
    private func _capturePicture(_ imageData: Data, _ imageCompletion: @escaping (CaptureResult) -> Void) {
    ...
    private func fixOrientation(withImage image: UIImage) -> UIImage {
        ...
        if image.imageOrientation != newOrientation, let cgImage = image.cgImage {
    ...
    public class CameraLocationManager: NSObject, CLLocationManagerDelegate {
        ...
        public func locationManager(_: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
            ...
            latestLocation = locations.min(by: { $0.horizontalAccuracy < $1.horizontalAccuracy })
        }
    ...
    private static var _blurSupported: Bool = {
        let supported: Set = [
            "iPad",
            "iPad1,1",
            "iPhone1,1",
    ...
    private func _updateCameraDevice(_: CameraDevice) {
        if let validCaptureSession = captureSession {
            validCaptureSession.beginConfiguration()
    ...
    private func _updateIlluminationMode(_ mode: CameraFlashMode) {
    ...
    private func _updateTorch(_: CameraFlashMode) {
        captureSession?.beginConfiguration()
    ...
    private func _updateFlash(_ flashMode: CameraFlashMode) {
        captureSession?.beginConfiguration()
        defer { captureSession?.commitConfiguration() }
        for captureDevice in AVCaptureDevice.videoDevices {
